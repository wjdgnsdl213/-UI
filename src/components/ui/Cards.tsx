...
export function KpiCard({ item, onClick }: { item: StatItem; onClick?: (targetId: string) => void }) {
  ...
  return (
    <button
      ...
      className={cn(
        'diagnosis-card anchor-target min-h-[148px] p-[18px] text-left transition',
    ...
    >
      ...
      <div className="text-[34px] font-black leading-none tracking-[-0.05em]">
        {item.value}
      ...
      {item.helperText ? <div className="mt-1 text-xs font-bold text-slate-500">{item.helperText}</div> : null}
      {item.basisText ? <div className="mt-2 rounded-xl bg-slate-50 px-3 py-2 text-[11px] font-bold leading-4 text-slate-500">기준: {item.basisText}</div> : null}
    </button>
  );
}
